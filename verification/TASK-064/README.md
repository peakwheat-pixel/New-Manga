...
被测提交：`e39cc27`（base `604c47a`），环境为 PowerShell 7 与 `TASK-012-py312`。

## 结果

- 定向：`tests/diagnostics tests/core`，`74 passed / 0 skipped`，`EXIT=0`。
- 全仓连续 5 次：每次 `950 collected / 944 passed / 6 skipped`，均 `EXIT=0`。
