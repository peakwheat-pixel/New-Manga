...
| AC-INPAINT-002 新 Clean 不覆盖旧版本 | P0 | **PASS** | 第二次 `REINPAINT_REGION` 生成 revision 2，revision 1 行与文件保留；`new_revision_required=True`；`ArtifactStepWriter` 发布路径为内容新 revision（不可覆盖） |
...
| AC-RFULL-001 完整链 | P0 | **BLOCKED（部分链路已打通）** | 实测：`REINPAINT_REGION` 的 `segment → mask_refine → inpaint` 全链通过；`RETRANSLATE_REGION` 的 `translate` 提交成功，`render` 在**规划阶段**即 `BLOCKED(missing_clean_artifact)`，无 StepRun。`color`/`term_extract`/`render` 的 handler **不在本 Task 允许路径**（`src/infrastructure/providers/**` 只覆盖 detection/OCR/translation/inpaint），故完整链不可在本 Task 内收口。解锁：Codex 明确 Render/Color/TermExtract handler 的归属与允许路径（见 Handoff 风险 R-2） |
