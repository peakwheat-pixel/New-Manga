...
| AC-RFULL-001 完整链 | P0 | **BLOCKED（handler 与生产装配面已打通；真实 OCR/翻译/修复能力仍缺）**【2026-09-18 TASK-033 更新口径】 | **装配面（TASK-033 证据）**：`color`/`term_extract`/`render` 三 handler 已接入 `src/infrastructure/providers/handlers.py` 并经 `src/bootstrap/app.py` 注入；渲染侧生产装配补齐（`RenderService` 在 `bootstrap.assemble_services` 构造，复用既有 `infrastructure/rendering/**` 适配器，无重型依赖可构建）；完整链 8 步（ocr→color→term_extract→translate→segment→mask_refine→inpaint→render）在真实 SQLite + 真实 seam + 替身 provider 下逐步 `COMPLETED`，`render` 提交 `translated` 制品且指针恰前进一次（`tests/providers/test_full_chain.py`）。替身 provider 只证明集成语义，**不作为模型质量证据**。TASK-019 本轮证据（`segment→mask_refine→inpaint` 全链、页面级累积）继续有效。**仍 BLOCKED 的项与解锁条件**：①真实 OCR 识别质量——解锁：依赖获批 + 权重路径 + hash 留档；②真实翻译质量/成本/时延——解锁：用户提供 OpenAI 兼容端点（或明确取消）；③真实修复（学习型路线）质量——解锁：`torch`/`diffusers`/`numpy` 依赖获批 + 权重 + 显存实测；④真实 Sakura 服务验证 NOT_RUN——解锁：本机启动 Sakura。**继承登记（非本切片引入）**：render-only 命令（`RERENDER_*`）跨 run 仍规划 `BLOCKED(missing_clean_artifact)`——`_clean_available` 依赖的 `clean` stage 无写入方，见 TASK-033 Handoff 遗留 |
...
| 完整链 `color`/`term_extract`/`render` handler 不在允许路径（TASK-019 本轮登记） | **已解除（TASK-033）** | 归属裁决与范围已由 TASK-033 落地（handlers + bootstrap 装配 + 完整链用例）；模型质量/端点项的 BLOCKED/NOT_RUN 状态不变（见 AC-RFULL-001 行的解锁条件） |
