...
class SettingsViewModel(QObject):
    ...
    def bindings(self) -> dict[str, str]:
        try:
            ...
            return {}
        return {
            step: _binding_profile_id(binding)
    ...
    def saveBinding(self, step_type: str, provider_profile_id: str) -> None:
        """Bind one pipeline step to a provider profile (AC-PROVIDER-002)."""
        try:
            self._defaults.save_provider_binding(step_type, provider_profile_id)
    ...
    def clearBinding(self, step_type: str) -> None:
        try:
            self._defaults.clear_provider_binding(step_type)
        except (SettingsError, BindingValidationError, ValueError, OSError) as error:
