...
class TrashService:
    ...
    def retry_pending_purges(self) -> int:
        ...
        for batch_id, targets in self._pending_purges():
            ...
            if self._batch_pages_still_alive(batch_id):
                continue
    ...
    def _batch_pages_still_alive(self, batch_id: str) -> bool:
        """Row-liveness guard for pending purges (TASK-060 Q-007 ③).

        True when the batch's pages are still live rows — the purge never
        reached the rows, so removing its files would orphan live pages.
        A batch only present as a rebuilt group counts as alive for the
        same reason (its rows exist); a batch whose rows are gone is the
        retryable case.
        """
        for batch in self.list_batches():
            if batch.batch_id == batch_id:
