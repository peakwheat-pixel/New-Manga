...
missing    尚未取得（本清单当前全部为此状态）
available  已取得、Hash 已记录、许可已确认
rejected   评估后不采用（保留记录，说明原因）
```

**Task AC2 要求"未取得素材标缺失"**：只要素材本体尚未生成或取得，状态只能是 `missing`；`planned` 仅用于方案已批准但仍属未取得的过渡记录，两者都不得用于任何 PASS 证据。

## 2. 来源类型与许可规则

| 来源类型 | 前缀写法 | 生成/取得要求 | 许可要求 |
|---|---|---|---|
| 项目生成（generated） | `generated / 自制（项目生成）` | 必须由确定性脚本生成（固定随机种子与参数），脚本随对应 Task 提交 | 无需外部许可；仍需记录脚本与 Hash |
| 外部取得（acquired） | `acquired / CC0`、`acquired / 用户授权`、`acquired / 待定` | 记录取得途径与版本 | **许可未确认（`待定`）时不得使用**，也不得标 `available` |

**`available` 的强制条件**（三项同时满足，缺一不可）：

1. `来源/许可` 明确且非 `待定`；
2. `Hash` 为真实 64 位小写十六进制 SHA256（不得为 `NOT_AVAILABLE` 或占位值）；
3. `生成方式` 可复现（脚本 + 参数，或取得途径 + 版本）。

未满足上述条件的条目必须保持 `missing`；把未取得素材标为 `available` 属于证据造假，验证脚本会拒绝。
...
| 未取得（missing） | 20 | 14 个功能素材 + 6 个数据集；均有生成方案但尚未执行 |
| 方案待批（planned） | 0 | 当前没有处于该状态的条目 |
| 已拒绝（rejected） | 0 | — |

素材取得与 Hash 登记由后续已授权 Task 执行；本文件当前不构成任何质量或性能证据，也不构成任何验收结果（结果以 [D13](../13_ACCEPTANCE_TRACEABILITY.md) 为准）。
