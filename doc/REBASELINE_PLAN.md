...
**M2 — Desktop Architecture Migration: PLANNED / NOT_RELEASED for implementation.** This future Milestone is not the current Active product Task. Its logical dependency order is AD-001 → Migration Discovery / Headless & Qt Coupling Audit → Python Core Decoupling → Contract / IPC → Tauri + React Skeleton → Incremental Feature Migration → Feature Parity → Tauri + Python Sidecar Packaging → Legacy QML Removal. Only [M2-D1 Headless & Qt Coupling Audit](tasks/M2-D1.md) is now `ready` as a read-only `research / audit` Task, based on the verified code baseline `307a940`; Owner=DeepSeek Harness, non-author Reviewer=Codex. Its write set is limited to its new Task, research report, Handoff and `verification/M2-D1/**` evidence. No React, Tauri, IPC, Python migration implementation, shared contract or Schema change is released. T3.2.1 remains the sole Active product Task.
