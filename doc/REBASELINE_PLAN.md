...
T3.2.1 remains the sole Active product Task with Full Release Gate OPEN. Child REPAIR-13 has an isolated delivery (`2f18e78`, Handoff `df95ad1`); after its first independent-review dispatch failed with DSH 401, Codex authorized a narrowly bounded R13-1 continuation for the unreadable ExportWindow and ChapterPicker controls. It is `in_progress` at author tip `33f2ff7`; no delivery is approved or integrated. The local probe and tests do not close the parent Gate: R13-AC3 still has unreadable dialog controls, the user has changed child R13-AC5 to a fixed-package GUI run on the current Windows host and that run is pending, and independent Review is pending. The previous child Sandbox attempt remains historical `BLOCKED` evidence; the parent T3.2.1 clean-Windows Gate is unchanged. Current Packaging/QML closure is Legacy Production Baseline work. A PyInstaller/QML Gate PASS will not be evidence of a future Tauri + Python Sidecar packaging PASS. Any reduced Legacy Closure scope is `USER_DECISION_REQUIRED`; until then the original Gate applies. React UI implementation ownership (Qoder, Zcode or per-feature allocation) is also `USER_DECISION_REQUIRED`; existing roles remain unchanged. A later, separately scoped Architecture Documentation Rebaseline Task should reconcile D02/D05/D07/D08 and `09_COLLABORATION` remote drift after AD-001 review, preserving historical records.
...
| 7 | [T3.2.1 Windows Packaging & Release Gate](tasks/T3.2.1.md) | **IN PROGRESS; FULL RELEASE GATE OPEN**; the historical fixed-package manual AC3 attempt was `BLOCKED` at image import; child [REPAIR-13](tasks/T3.2.1-REPAIR-13.md) is `in_progress` after Delivery `2f18e78` / Handoff `df95ad1`, with R13-1 scope extension authorized at tip `33f2ff7` | Codex owns Gate closeout/integration; ZCode owns the bounded QML repair; DeepSeek Harness is independent Reviewer | Product onedir must launch on clean Windows without Python, complete the core workflow, protect user/source data, and exit without a residual process. The isolated local probe covers import/export paths but does not establish the required fixed-package GUI workflow; ExportWindow/ChapterPicker readability, the child local run, `git diff --check` and independent Review remain open. The child run must launch the fixed delivery EXE on the current host with isolated temporary data and complete the real GUI flow; it does not establish the parent clean-Windows Gate. AC3 remains a release blocker until the parent’s full interactive run succeeds. See [Codex closeout](../verification/T3.2.1/release-gate-closeout-b800d11.md), [AC3 manual observations](../verification/T3.2.1/ac3-interactive-20260925/manual-observations.md); REPAIR-13 evidence remains in its isolated ZCode worktree at `verification/T3.2.1/repair-13/`. |
...
T1.2.1 VERIFIED_COMPLETE
  ...
  → T3.2.1 full Release Gate OPEN; AC3 BLOCKED pending complete fixed-package workflow
  → REPAIR-13 in progress; R13-1 dialog readability continuation from `33f2ff7`
  → child fixed-package EXE GUI run on current host + independent Review → Codex integration evidence (not yet authorized)
