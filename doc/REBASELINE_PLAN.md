...
| 7 | [T3.2.1 Windows Packaging & Release Gate](tasks/T3.2.1.md) | **IN PROGRESS; FULL RELEASE GATE OPEN**; REPAIR-12 evidence slice Review `a656486` is `approved`; fixed-package manual AC3 attempt is `BLOCKED` at image import | Codex owns Gate closeout and integration; DeepSeek Harness is the required independent final Reviewer | Product onedir must launch on clean Windows without Python, complete the core workflow, protect user/source data, and exit without a residual process. AC3 remains a release blocker: the GUI launched, but image import hit a QML scope error, so the full interactive workflow is not complete. A separately registered repair and fresh full test are required. See [Codex closeout](../verification/T3.2.1/release-gate-closeout-b800d11.md) and [AC3 manual observations](../verification/T3.2.1/ac3-interactive-20260925/manual-observations.md). |
...
T1.2.1 VERIFIED_COMPLETE
  ...
  → T3.2.1 full Release Gate OPEN; AC3 interactive workflow BLOCKED at image import
