...
T3.2.1 remains the sole Active product Task with Full Release Gate OPEN. Child [REPAIR-13](tasks/T3.2.1-REPAIR-13.md) is `in_review` under ZCode at fixed base `8738c41`, product delivery `aed0094`, Handoff/evidence head `66a13f8`; DeepSeek Harness Review `ccd26a4` is `APPROVE_WITH_CONDITIONS`. The three authorization phases and F-13-4 inclusion are registered in the Task; child AC4=`PASS`, and child AC1–AC5 are accepted under the user's local fixed-package verification ruling. Conditioned evidence cleanup F-1/F-3 is assigned to a ZCode evidence-only follow-up; no product code is integrated, and no product branch is to be merged to `master` before Codex integration. The former Sandbox attempts remain historical evidence; the parent T3.2.1 clean-Windows/Sandbox Gate is unchanged. [REPAIR-14](tasks/T3.2.1-REPAIR-14.md) is proposed with Owner=Antigravity for F-13-3 (bookshelf empty after restart), depends on REPAIR-13 completion, and has no implementation release or fixed base. Current Packaging/QML closure is Legacy Production Baseline work. A PyInstaller/QML Gate PASS will not be evidence of a future Tauri + Python Sidecar packaging PASS. Any reduced Legacy Closure scope is `USER_DECISION_REQUIRED`; until then the original Gate applies. React UI implementation ownership (Qoder, Zcode or per-feature allocation) is also `USER_DECISION_REQUIRED`; existing roles remain unchanged. A later, separately scoped Architecture Documentation Rebaseline Task should reconcile D02/D05/D07/D08 and `09_COLLABORATION` remote drift after AD-001 review, preserving historical records.
...
| 7 | [T3.2.1 Windows Packaging & Release Gate](tasks/T3.2.1.md) | **IN PROGRESS; FULL RELEASE GATE OPEN**; parent AC3 remains `BLOCKED`; child [REPAIR-13](tasks/T3.2.1-REPAIR-13.md) is `in_review` at delivery `aed0094` / evidence head `66a13f8`, Review `APPROVE_WITH_CONDITIONS`; child AC4=`PASS`, F-1/F-3 evidence cleanup pending. Proposed child [REPAIR-14](tasks/T3.2.1-REPAIR-14.md) is not released. | Codex owns Gate closeout/integration; ZCode owns REPAIR-13; Antigravity is proposed Owner for F-13-3; DeepSeek Harness is independent Reviewer | Product onedir must launch on clean Windows without Python, complete the core workflow, protect user/source data, and exit without a residual process. REPAIR-13's child local fixed-package GUI flow and two-page export are accepted for its child scope; they do not establish the parent clean-Windows Gate. Full parent AC3 and remaining Gate criteria stay open. See [Codex closeout](../verification/T3.2.1/release-gate-closeout-b800d11.md), [AC3 manual observations](../verification/T3.2.1/ac3-interactive-20260925/manual-observations.md); REPAIR-13 verification and Review are linked from its Task. |
...
T1.2.1 VERIFIED_COMPLETE
  ...
  → REPAIR-13 in_review @ delivery `aed0094`; AC4 PASS; Review `APPROVE_WITH_CONDITIONS`
  → ZCode evidence-only F-1/F-3 cleanup → Codex integration decision (product branch not merged)
  → proposed REPAIR-14 (Antigravity) for F-13-3, not released
