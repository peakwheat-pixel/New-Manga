...
**M2 — Desktop Architecture Migration: PLANNED / NOT_RELEASED.** This future Milestone is not the current Active Task. Its logical dependency order is AD-001 → Migration Discovery / Headless & Qt Coupling Audit → Python Core Decoupling → Contract / IPC → Tauri + React Skeleton → Incremental Feature Migration → Feature Parity → Tauri + Python Sidecar Packaging → Legacy QML Removal. This sequence does not release any React, Tauri, IPC or Python migration implementation. The only successor proposed now is a `research / audit` task, Headless & Qt Coupling Audit; it is not ready or authorized to start. Codex must later freeze its base, paths, Owner and Reviewer before release.

T3.2.1 remains the sole Active product Task with Full Release Gate OPEN and REPAIR-13 ready. Current Packaging/QML closure is Legacy Production Baseline work. A PyInstaller/QML Gate PASS will not be evidence of a future Tauri + Python Sidecar packaging PASS. Any reduced Legacy Closure scope is `USER_DECISION_REQUIRED`; until then the original Gate applies. React UI implementation ownership (Qoder, Zcode or per-feature allocation) is also `USER_DECISION_REQUIRED`; existing roles remain unchanged. A later, separately scoped Architecture Documentation Rebaseline Task should reconcile D02/D05/D07/D08 and `09_COLLABORATION` remote drift after AD-001 review, preserving historical records.
...
| 7 | [T3.2.1 Windows Packaging & Release Gate](tasks/T3.2.1.md) | **IN PROGRESS; FULL RELEASE GATE OPEN**; fixed-package manual AC3 attempt is `BLOCKED` at image import; child [REPAIR-13](tasks/T3.2.1-REPAIR-13.md) is `ready` | Codex owns Gate closeout/integration; Qoder owns the bounded QML repair; DeepSeek Harness is independent Reviewer | Product onedir must launch on clean Windows without Python, complete the core workflow, protect user/source data, and exit without a residual process. REPAIR-13 fixes the chapter-import entry and low-contrast controls, then tests Export only with valid imported pages and chapter selection. AC3 remains a release blocker until a fresh full interactive run succeeds. See [Codex closeout](../verification/T3.2.1/release-gate-closeout-b800d11.md) and [AC3 manual observations](../verification/T3.2.1/ac3-interactive-20260925/manual-observations.md). |
...
T1.2.1 VERIFIED_COMPLETE
  ...
  → T3.2.1 full Release Gate OPEN; AC3 BLOCKED at image import
  → REPAIR-13 QML import/readability repair → fresh fixed-package Sandbox AC3 run
  → final independent Review → Codex integration evidence (not yet authorized)
```
