...
- **AC②（解析一致性 + 判别力）**：`remove_managed` 新增第三层守卫——沿**未解析**路径逐级 `lstat`，任一级为 reparse point（Windows `FILE_ATTRIBUTE_REPARSE_POINT` / POSIX `S_ISLNK`）→ `ImmutablePathViolation`（typed）。判别用例 `TestR010ReparseWalk`：`mklink /J` 根内目录 junction → 同根另一 chapter 的受保护原件，修前删除成功（目标消失）、修后 typed 拒绝且**目标存活**。声明限制：仅覆盖目录 junction 形态（`mklink /J` 无需特权）；文件符号链接需开发者模式/特权，未覆盖；POSIX symlink 走 `S_ISLNK` 分支在本机未实测。
...
| AC①④ 单元判别（修前） | 同上树跑 `tests/core/test_connection_ownership.py` | 同上 @ 6cb0afb | **3 failed（预期）**：`ThreadRoutedConnection` 无 `registry_size/release_current_thread_connection/any_in_transaction`，EXIT=1 | `verification/TASK-061/prefix-discrimination/connection-ownership-unit-prefix-run1.log` |
| AC②③ 判别（修前） | 同上树跑 `tests/storage/test_managed_storage.py` | 同上 @ 6cb0afb | **3 failed（预期）**：junction 删除成功（受保护原件消失）、`.`/空段删除成功；既有 7 例 passed，EXIT=1 | `verification/TASK-061/prefix-discrimination/managed-storage-traverse-prefix-run1.log` |
...
- 第一次修前树全文件跑曾偶发挂起（subprocess cmd/mklink 疑似沙箱瞬时问题）；复跑两次均正常（0.6s 内完成），已 Stop 僵留进程后重新取证——记录于此供 Reviewer 知情。
