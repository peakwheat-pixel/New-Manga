...
| W11 | TASK-058 | uphold_with_findings | drain 超时早退后无条件 `conn.close()`，与其 docstring 自相矛盾（Q-003） |
...
| Q-011 | P3 | W5/W7/W8/W11 文档与 QML 细节 | `WorkbenchView.qml:57,80-135` 3 处硬编码颜色绕过 token 系统（喂给 TASK-047/059 设计门）；`TASK-054.md`/`TASK-055.md` 的 AC 勾选框在 `status: done` 下仍为 `[ ]`；`TASK-058.md` 引 `src/workbench/run_controller.py:96`（真实路径 `src/ui/viewmodels/workbench/run_controller.py`）；`56a429e`/`7f13e53` 各把误名空文件提交进实现提交（后一提交删除） | [读码]/[子代理] | 随下一次文档收口一并清理 | open（不阻断） |
