...
耗时为该路线 3 次重复的 **min/max**；`ink` 为修复前 Mask 内的暗像素数；`residual` 为修复后仍是暗像素的数量（**残字的像素统计代理，不是视觉质量判定**）；`viol` 为 **final Mask 外被改动的像素数**（必须为 0；该判据的覆盖范围严格包含 `samples/manifest.json` 声明的保护框，但 harness 自修订切片 `d7c10d4` 起另有 `protected_box_violations` 逐框断言，20/20 框全为 0，见 §9.1）；`rss` 为进程峰值工作集。

| 样例 | 路线 | min ms | max ms | ink(前) | residual(后) | viol | peak RSS MB |
|---|---|---:|---:|---:|---:|---:|---:|
| white-background | simple-fill | 2.203 | 3.054 | 519 | **0** | **0** | 46.13 |
| white-background | edge-bleed | 40.194 | 43.846 | 519 | 519 | **0** | 50.83 |
| line-art | simple-fill | 2.3 | 2.606 | 2010 | **0** | **0** | 54.48 |
| line-art | edge-bleed | 32.282 | 38.306 | 2010 | 1932 | **0** | 56.11 |
| screentone | simple-fill | 2.244 | 2.41 | 1794 | **0** | **0** | 56.11 |
| screentone | edge-bleed | 40.711 | 44.354 | 1794 | 1410 | **0** | 57.55 |
| gradient | simple-fill | 2.284 | 2.41 | 570 | **0** | **0** | 57.55 |
| gradient | edge-bleed | 39.071 | 43.466 | 570 | 570 | **0** | 57.55 |
| structure-crossing | simple-fill | 2.372 | 2.593 | 2958 | **0** | **0** | 57.55 |
| structure-crossing | edge-bleed | 44.075 | 48.609 | 2958 | 2031 | **0** | 57.55 |

> **数据归属（Review R-102）**：上表 `min ms` / `max ms` / `peak RSS MB` 三列取自**当前仓库内** `experiments/TASK-018/results/experiment.json`（`schema = task018-experiment-v2`，由修订切片 `d7c10d4` 生成）；`ink(前)` / `residual(后)` / `viol` 三列为首轮与本次**逐值相同**的确定性量。
> 首轮交付 `6c33e7f` 的 `experiment.json` 中，**耗时与 RSS 与上表不同**（这两项是易变量，随机器负载浮动）。**引用本表数字时，其所属 artifact 是 `d7c10d4` 的 `results/experiment.json`**。
...
5. **资源要求**：本环境实测基线峰值 RSS **≤ 57.6 MB**、CPU 耗时 **≤ 48.7 ms/320×320**（取自 `d7c10d4` 的 `experiment.json`）。**学习型路线的显存/内存需求在本环境完全未测**（`BLOCKED`），因此**不得**为它们写入任何资源数字；TASK-019 若需要，必须先在实际具备依赖与权重的环境重跑本实验。
...
| OOM 行为 | **NOT_RUN（无法触发）** | 唯一可运行的路线是 CPU 常量填充/扩散，峰值 57.6 MB，**无法构造真实 OOM**；学习型路线的 OOM 行为随其 `BLOCKED` 一并未知 |
