...
U-5 已批准用户上传字体单文件上限 50 MB、总数上限 200 个，以及本地使用许可声明文案；该决定不释放 TASK-022。
U-6 已批准 Sakura 监控仅做健康探测和就绪状态，不做显存或负载等深度指标；该决定不释放 TASK-019。
...
| 释放条件 | U-6 范围已批准；TASK-018 已由用户释放为 ready、尚未完成；TASK-019 仍 blocked 且未获单独释放。TASK-017 真实端点层仍 NOT_RUN；TASK-019 满足全部硬依赖并获单独释放后，由 Codex 明确 AC/allowed_paths。运行测试需本地 Sakura 实例；环境不具备时按 D08 记录 BLOCKED/NOT_RUN |
...
| U-5 | **已决定：单个上传字体文件 ≤50 MB、用户上传字体总数 ≤200；提示文案为“上传即声明本人已具备该字体的本地使用许可；用户自行确保许可，应用不分发字体。”** | 上限与许可提示已确定；TASK-022 未释放，依赖、AC 与 allowed_paths 仍待落实 |
| U-6 | **已决定：Sakura 监控仅做健康探测和就绪状态，不做显存或负载等深度指标** | 监控范围已定；TASK-019 仍 blocked，未获释放且依赖/真实端点验证条件未满足 |
