...
> **Architecture Decision（2026-09-25）**：[AD-001 Target Desktop Architecture](AD-001_TARGET_DESKTOP_ARCHITECTURE.md) 已记录用户批准的未来 React + TypeScript + Tauri + Python Core 目标，状态为 `ACCEPTED_TARGET_ARCHITECTURE / IMPLEMENTATION_NOT_RELEASED`。当前生产仍是 Python + PySide6/QML + SQLite；M2 实现仍 `PLANNED / NOT_RELEASED`；只读研究 [M2-D1 Headless & Qt Coupling Audit](tasks/M2-D1.md) 已以 `ready` 释放给 DeepSeek Harness，Codex 非作者 Review；唯一 Active 产品 Task 仍是 T3.2.1。登记与集成记录见 [GOV-001](tasks/GOV-001.md)（`done`，merge `969f7a5e`），[DeepSeek Harness 独立 Review](reviews/GOV-001-2e8f4bf.md) `bbf24e73` 为 `approved`；排期权威仍为 [REBASELINE_PLAN](REBASELINE_PLAN.md)，运行状态仍为 [STATUS](STATUS.md)。
