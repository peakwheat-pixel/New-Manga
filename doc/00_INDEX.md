...
> **Current pointer（2026-09-25）**：T1.1.1、T1.1.2、T1.2.1、T2.1.1、T2.2.1、T3.1.1 已集成并验证；T3.1.1 产品 head=`0c70e44`，独立 Review=`4e1eb3a`，集成证据为 [integration-899bd3c](../verification/T3.1.1/integration-899bd3c.md)。T3.2.1 全量 Release Gate 仍 OPEN；REPAIR-12 Review `a656486` 对证据修订切片 `approved`，不代表全量 Gate 通过。固定 Build 3 的真实 GUI 尝试在图片导入处 `BLOCKED`，AC3 仍是发布阻断项；REPAIR-13 按用户改派由 ZCode 承接（base `8738c41`，DeepSeek Harness 独立复审）。AC5=`NOT_RUN`、AC6/AC7=`PARTIAL / NOT_RUN`、AC8/AC9=`NOT_RUN`、R-014～R-016=`OPEN`。导入 TypeError、导出按钮的禁用前置状态及截图证据见 [AC3 manual observations](../verification/T3.2.1/ac3-interactive-20260925/manual-observations.md)；修复范围见 [REPAIR-13 Task](tasks/T3.2.1-REPAIR-13.md)。O-12-2 的处置及固定检查见 [Codex closeout](../verification/T3.2.1/release-gate-closeout-b800d11.md)。当前状态与执行恢复点见 [STATUS](STATUS.md)；任务顺序见 [REBASELINE_PLAN](REBASELINE_PLAN.md)。下方 2026-09-19 及更早的长段落是历史导航，不决定当前排期。
