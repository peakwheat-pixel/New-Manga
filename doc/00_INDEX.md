...
> **Current pointer（2026-09-25）**：T1.1.1、T1.1.2、T1.2.1、T2.1.1、T2.2.1、T3.1.1 已集成并验证；T3.1.1 产品 head=`0c70e44`，独立 Review=`4e1eb3a`，集成证据为 [integration-899bd3c](../verification/T3.1.1/integration-899bd3c.md)。T3.2.1 全量 Release Gate 仍 OPEN；REPAIR-12 Review `a656486` 对证据修订切片 `approved`，不代表全量 Gate 通过。REPAIR-13 的固定 base 为 `8738c41`，Delivery=`2f18e78`、Handoff=`df95ad1`；ZCode worktree 在 `33f2ff7`，首次 DSH Review 派发以 401 失败，未生成报告。Codex 已授权 R13-1 只修 `ExportWindow.qml` / `ChapterPicker.qml` 的按钮文字对比度；当前状态为 `in_progress`。本地组装探针/测试不能代替固定包 GUI 验证：用户已将 REPAIR-13 R13-AC5 改为当前 Windows 主机运行固定 delivery EXE（临时数据根隔离、真实 GUI 全流程），该项待执行；导入 UI 文件选择器与 Workbench 两页选择仍未验证，截图 08/09 显示相关对话框按钮仍低对比，此前 Sandbox 子项尝试为历史 `BLOCKED`，完整 `git diff --check` 因原始判别日志空格退出非零。此变更不替代父 T3.2.1 的现有干净 Windows / Sandbox Gate；父 Gate 仍 OPEN、AC3 仍 BLOCKED、AC5=`NOT_RUN`、AC6/AC7=`PARTIAL / NOT_RUN`、AC8/AC9=`NOT_RUN`、R-014～R-016=`OPEN`。证据见 [REPAIR-13 Task](tasks/T3.2.1-REPAIR-13.md)；Handoff `doc/handoffs/T3.2.1-REPAIR-13-2f18e78.md` 与 verification 位于 ZCode worktree 的 `verification/T3.2.1/repair-13/`，尚未集成。原始 Build 3 失败证据仍在 [AC3 manual observations](../verification/T3.2.1/ac3-interactive-20260925/manual-observations.md)；O-12-2 的处置及固定检查见 [Codex closeout](../verification/T3.2.1/release-gate-closeout-b800d11.md)。当前状态与执行恢复点见 [STATUS](STATUS.md)；任务顺序见 [REBASELINE_PLAN](REBASELINE_PLAN.md)。下方 2026-09-19 及更早的长段落是历史导航，不决定当前排期。

> **Architecture Decision（2026-09-25）**：[AD-001 Target Desktop Architecture](AD-001_TARGET_DESKTOP_ARCHITECTURE.md) 已记录用户批准的未来 React + TypeScript + Tauri + Python Core 目标，状态为 `ACCEPTED_TARGET_ARCHITECTURE / IMPLEMENTATION_NOT_RELEASED`。当前生产仍是 Python + PySide6/QML + SQLite；M2 实现仍 `PLANNED / NOT_RELEASED`。只读研究 [M2-D1 Headless & Qt Coupling Audit](tasks/M2-D1.md) 已完成，[报告](research/M2-D1_HEADLESS_QT_COUPLING_AUDIT.md)与 [Codex 非作者 Review](reviews/M2-D1-0a27a54.md) 已集成（merge `100abfb`）；未释放后续迁移实现。唯一 Active 产品 Task 仍是 T3.2.1。架构登记见 [GOV-001](tasks/GOV-001.md)（`done`，merge `969f7a5e`）及其 [DeepSeek Harness 独立 Review](reviews/GOV-001-2e8f4bf.md)；排期权威仍为 [REBASELINE_PLAN](REBASELINE_PLAN.md)，运行状态仍为 [STATUS](STATUS.md)。
