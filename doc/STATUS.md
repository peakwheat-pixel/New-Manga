...
| Clean Windows environment route | User has no second computer. Candidate is an independent Windows 11 Hyper-V VM on this host; virtualization and Hyper-V services are active, but this non-elevated account cannot list VMs. User's next read-only check is elevated `Get-VM | Select Name,State`; VM availability and AC3 remain unverified. |
