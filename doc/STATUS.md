...
| Clean Windows environment route | User has no second computer; elevated `Get-VM | Select Name,State` returned no VM twice. Windows Sandbox enablement failed; host CBS.log at 2026-09-23 21:27:57 reports component-store corruption `0x80073712` during optional-feature servicing and rolls `Containers-DisposableClientVM` back Off. Host repair is pending; no isolated test environment has run, and AC3 remains NOT_RUN. A full Hyper-V VM remains fallback. |
