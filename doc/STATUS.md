...
| Clean Windows environment route | User has no second computer. Elevated `Get-VM | Select Name,State` returned no VM twice. Windows Sandbox is the simpler same-host clean-desktop candidate, pending feature enablement and suitability check against AC3/AC6/AC7; a full Hyper-V VM remains fallback. Neither has been tested, and AC3 remains NOT_RUN. |
