...
| Clean Windows environment route | User has no second computer. After successful DISM/SFC repair plus reboot, Sandbox still fails with `0x80073712`; isolated enable attempt failed too. CBS 22:32:02 pinpoints `CRawStoreLayout::RecursivelyRegenerateComponentPayload` corruption. On Windows 11 Pro 25H2 x64 build 26200.9457, next try Settings > System > Recovery > Fix problems using Windows Update > Reinstall now. If unavailable or unsuccessful, use matching official media or resume Hyper-V VM route (`vmms`/`vmcompute` running; no VM exists). No isolated test environment has run; AC3 remains NOT_RUN. |
