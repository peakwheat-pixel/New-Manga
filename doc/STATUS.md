...
| Clean Windows environment route | User has no second computer; elevated `Get-VM | Select Name,State` returned no VM twice. After successful DISM repair and SFC repair plus reboot, Windows Sandbox enablement via Windows Features still failed; CBS.log at 2026-09-23 22:28:36 again reports component-store error `0x80073712`. Next isolate Sandbox with the single-feature PowerShell command. No isolated test environment has run; AC3 remains NOT_RUN. A full Hyper-V VM remains fallback. |
