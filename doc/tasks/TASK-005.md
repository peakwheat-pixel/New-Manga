...
本 Task 已获用户授权，方案 A 与书面规格均已批准；首次独立 Review 的 P1 已按 TDD 修订，当前以新固定 head 进入复审。本 Task 不释放 TASK-006～TASK-027。当前阶段见 [STATUS](../STATUS.md)；共用流程见 [协作协议](../09_COLLABORATION.md)。
...
- [x] 建立 Domain 禁止导入 Qt/SQLite/ML、UI 禁止直连具体存储/Provider 的可运行守卫。（R-001 已补 `ImportFrom.module=None` 相对 alias 回归；完整 core 6 passed，等待独立复审确认）
...
- 本守卫用于阻止常见静态 import 违规，不是 Python 安全边界；`importlib.import_module` / `__import__` 等动态导入不在 TASK-005 授权范围，后续若出现真实用例再扩展。
...
- Review：DeepSeek Harness 对首次 head 给出 [changes_requested](../reviews/TASK-005-e914f39.md)：P0=0、P1=1、P2=3；新 head 等待复审，报告路径 `doc/reviews/TASK-005-f343008.md`。
- 实际执行/实验/测试：[作者验证](../../verification/TASK-005/author-verification.md)；新固定 head 复跑 `verify.ps1` 退出码 0，6 passed。
- 最近状态：2026-09-14 新 `reviewed_head=f3430080d88f07d789c21e76d5b5b4310fbf3458` 已固定。R-001/R-002/R-003 fixed；R-004 deferred（全仓只有 CLI 调用，应用参数由 argparse 消费，不转交 Qt；出现程序化/Qt 参数调用者时再定义接口）。状态 `changes_requested → in_review`；TASK-006～TASK-027 未释放。
