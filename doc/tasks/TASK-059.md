...
- [x] **AC ④（硬事实沿用或显式推翻）**：DPI/最小窗口组合、文本对比度 ≥4.5:1、CJK 长文本与截断、状态矩阵（空/加载/错误/运行/未保存）、键盘可达性：要么沿用 TASK-047 §10 的量化结论，要么**显式推翻并给新证据**。→ 契约 §12 对照表：硬事实沿用、ui-baseline 色板作为目标方向显式推翻（授权内）；对比度 50 组合 × 22 对全 PASS（R2 含 2 处审计驱动修正；R3 补徽标真实合成底口径后统一加深亮色状态文字令牌，见契约 §7.2）。
...
- [x] **AC ⑥（交付与流程）**：交付本轮契约（**替代** TASK-047 的契约，不是在其上补丁）、候选方向参考、`verification/TASK-059/**`（自检脚本 + 审计 JSON + 截图）、Handoff；经**非作者** Review（Codex）与集成后才能 done；**设计身份不自动授予 `src/**` 写权限**（实现另行释放实现 Task）。→ 契约新写（非补丁）；`verification/TASK-059/`（脚本+审计结果+41 截图+令牌导出+判别脚本）；[Handoff](../handoffs/TASK-059-three-candidates.md)；`src/**` 零改动。
...
|---|---|---|---|---|
| AC ②③ 参考可加载 | 无头浏览器打开每个候选参考；桌面 1280×800 与 100/150/200% DPI 截图；检查溢出/重叠/控制台错误 | 仓库 base `4c81dca` + 无头 Chrome + Node（本切片不跑 pytest） | **executed**（R3 复跑）：语法门 OK；41 张截图（含 DPI 125/150/200 与玻璃开关）；50 组合审计 outside=0/clipped=0/**overlap=0（真实两两相交检测）** | [audit-result.txt](../../verification/TASK-059/audit-result.txt)（同日志含环境头/命令/`AUDIT_EXIT=0`）、[screenshots/](../../verification/TASK-059/screenshots/) |
| AC ④ 硬事实 | 对比度审计（≥4.5:1，15 直底对 + 7 徽标 `*-soft` 真实合成底对）+ 判别力脚本（回退旧令牌应 FAIL、注入相交元素应 FAIL）+ CJK/截断/状态矩阵逐项截图 | 同上 + `TASK-012-py312` venv（导出脚本用） | **executed**（R3）：5 候选 × 2 主题 × 5 视图 × 22 对全 PASS（R3 修正：亮色状态文字令牌统一加深，见契约 §7.2）；判别 D1=`badge-warn/st-warn-t=4.22` FAIL、D2=`overlap=1` 检出 → 检测有判别力；CJK 竖排缺陷修复后复检 PASS | 同上 + [判别脚本](../../verification/TASK-059/discriminate-r001-r004.sh) + [契约 §7.2/§12](../contracts/UI_UX_GUI_DESIGN.md) |
| AC ⑥ 文档完整性 | `git diff --check`；链接与 AC/Gap/待决项逐条核对 | 同上 | **executed**：`git diff --check` 退出码 0；契约链接/截图引用/JSON↔HTML 一致性交叉核对 ALL OK（R1 建立，R3 复核口径行） | [Handoff 验证证据表](../handoffs/TASK-059-three-candidates.md) |
...
- Handoff：[TASK-059-three-candidates](../handoffs/TASK-059-three-candidates.md)。Review：[TASK-059-db366da](../reviews/TASK-059-db366da.md)（Reviewer=`Codex`，非作者，`changes_requested`，R-001～R-006）→ **R3 返修（当前）**。
- **最近状态（当前，唯一）**：2026-09-19 **R3 返修交付，保持 `in_review`**。按 Review `db366da` 逐条处理：**R-001** 徽标对比度改按真实 `*-soft` 合成底审计（7 对新徽标对），五候选亮色状态文字令牌统一加深一档（soft alpha 与暗色不动），修正后 50 组合 × 22 对全 PASS；**R-002** 契约 §6B 补命令面板完整契约（命令模型/VM/状态与失败语义/快捷键与焦点/IME/危险动作/objectName/测试切面），§10 显式例外消解矛盾；**R-003** `S-SHELF-BOOK`/`S-WB-INSPECTOR` 扩展到 D/E 并补切片边界注记；**R-004** `layoutAudit()` 实现真实同级两两相交检测并输出相交元素对，审计脚本判定同步；**R-005** 全量审计按 Git Bash + `TASK-012-py312` 口径重跑，环境头/命令/`AUDIT_EXIT=0` 同日志入库，Handoff 不实声明已更正；**R-006** R1 计数残留全量清理（五候选/50 组合/41 截图/DDR-1 注 DDR-8/脚本头/导出脚本/D-E JSON 玻璃说明）。判别力证据：[discriminate-r001-r004.sh](../../verification/TASK-059/discriminate-r001-r004.sh)（回退旧令牌 → `badge-warn=4.22` FAIL；注入相交徽标 → `overlap=1` 检出）。R3 head=本次提交。**待 Codex 复审；候选方向选择（契约 §13 ND-1，含 ND-8/9/10）仍待用户裁决。**
- 历史：2026-09-19 R2 交付完成（R1 交付 A/B/C 三候选，实现 head=`5578a01`、契约 head=`ddaf865`；用户在 R1 交付后追加指示「再增加两种完全不一样的设计」→ R2 增补 D · Vermilion 朱砂与 E · Amber 琥珀，R2 head=`dd455c0`（参考+证据）、`af91e06`（契约），候选总数 3→5（AC② 口径经用户指示扩展，DDR-8）；R2 证据＝审计矩阵 50 组合 ALL PASS + 41 张截图）。同日 Review `db366da` 判 `changes_requested`（R-001 徽标对比度审计方式错误、R-002 命令面板契约缺口、R-003 D/E 切片缺口、R-004 overlap 未实现、R-005 审计日志证据纪律、R-006 R1 残留）→ R3 返修。base=`4c81dca`、branch=`agent/zcode/TASK-059-ui-redesign`、worktree=`G:/CODEX/New Manga.worktrees/TASK-059-zcode`。
