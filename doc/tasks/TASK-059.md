...
- [x] **AC ⑥（交付与流程）**：交付本轮契约（**替代** TASK-047 的契约，不是在其上补丁）、候选方向参考、`verification/TASK-059/**`（自检脚本 + 审计 JSON + 截图）、Handoff；经**非作者** Review（Codex）与集成后才能 done；**设计身份不自动授予 `src/**` 写权限**（实现另行释放实现 Task）。→ 契约新写（非补丁）；`verification/TASK-059/`（脚本+审计结果+49 截图（R1–R3 41 + R4 F 8）+令牌导出+判别脚本）；[Handoff](../handoffs/TASK-059-three-candidates.md)；`src/**` 零改动。
...
|---|---|---|---|---|
| AC ②③ 参考可加载 | 无头浏览器打开每个候选参考；桌面 1280×800 与 100/150/200% DPI 截图；检查溢出/重叠/控制台错误 | 仓库 base `4c81dca` + 无头 Chrome + Node（本切片不跑 pytest） | **executed**（R4 复跑）：语法门 OK；49 张截图（6 候选；含 DPI 125/150/200、玻璃开关与 F 任务详情工具窗）；60 组合审计 outside=0/clipped=0/**overlap=0（真实两两相交检测）** | [audit-result.txt](../../verification/TASK-059/audit-result.txt)（同日志含环境头/命令/`AUDIT_EXIT=0`）、[screenshots/](../../verification/TASK-059/screenshots/) |
| AC ④ 硬事实 | 对比度审计（≥4.5:1，15 直底对 + 7 徽标 `*-soft` 真实合成底对）+ 判别力脚本（回退旧令牌应 FAIL、注入相交元素应 FAIL）+ CJK/截断/状态矩阵逐项截图 | 同上 + `TASK-012-py312` venv（导出脚本用） | **executed**（R4）：6 候选 × 2 主题 × 5 视图全 PASS——A–E 各 22 对、F 24 对（新增 R3-001 扩展对 `st-skip-direct/panel`=7.22、`ink/selected-row`=11.34，阈值 4.5:1）；判别 D1=`badge-warn/st-warn-t=4.22` FAIL、D2=`overlap=1` 检出、D3=回退 F 暗色 `--st-skip` → `st-skip-direct/panel=3.01` FAIL → 检测有判别力；CJK 竖排缺陷修复后复检 PASS | 同上 + [判别脚本](../../verification/TASK-059/discriminate-r001-r004.sh) + [契约 §7.2/§12](../contracts/UI_UX_GUI_DESIGN.md) |
...
- Handoff：[TASK-059-three-candidates](../handoffs/TASK-059-three-candidates.md)。Review：[TASK-059-db366da](../reviews/TASK-059-db366da.md)（Reviewer=`Codex`，非作者，`changes_requested`，R-001～R-006）→ R3 返修（head=`07397ac`）→ [TASK-059-07397ac](../reviews/TASK-059-07397ac.md)（分支 `agent/codex/TASK-059-review` @ `9d3b0f2`，`approved`，非阻塞 R3-001）→ **R4 组合候选 F（当前）**。
- **最近状态（当前，唯一）**：2026-09-19 **R4 组合候选 F 交付，保持 `in_review`**。Review `07397ac` 复审 `approved`（唯一非阻塞 P2 = R3-001）后，用户作出 ND-1 裁决：配色/状态色/亮暗主题/视觉语言＝**A · Graphite**，字号/控件高/行高/间距/面板尺寸＝**B · Atelier**，IA＝**A**（不继承 B 章节大卡），不启用 C 玻璃/D 顶栏/E 命令面板。按契约 ND-1「组合稿必须重做全套一致性审计」条款交付：① `ui-reference.html` 新增候选 **F · Graphite Atelier**（几何块逐项取 B 的 20 项密度令牌、色板逐字面值取 A、书架 `--card-w` 取 A 档、无任何 C/D/E 覆写）；② 新增 `doc/design/tokens-cand-f.json`（42 色 × 2 主题 + 20 几何；python 断言 `F = A color palette + B geometry: OK`，glass `enabled=false`）；③ 解决 **R3-001**：`tokenPairs()` 仅 F 追加扩展对 `st-skip-direct/panel`（`--st-skip` 直接文本 × panel）与 `ink/selected-row`（选中行 `accent-soft` 叠 panel 合成底 × `--ink`），实测 **7.22 / 11.34 ≥ 4.5:1 PASS**，A–E 维持 22 对口径不变；④ 审计矩阵重跑 **60 组合（6 候选 × 2 主题 × 5 视图）ALL PASS**（F=24/24、outside/clipped/overlap=0，`AUDIT_EXIT=0`）；截图 41→49 张（+8 张 F，含 DPI 150 与任务详情工具窗）；⑤ 判别 **D1/D2/D3 全 FAIL（预期方向）**，D3=注入回退 F 暗色 `--st-skip` → `st-skip-direct/panel=3.01` 检出（`DISCRIM_EXIT=0`）；⑥ 契约全面更新（§3 六候选总览、§6C F 规格、§7/§8/§9/§10/§11 切片映射、§13 ND 状态、§14 DDR-9、§15 证据索引）。A–E 保留为候选档案；改动仍仅 `doc/**` + `verification/TASK-059/**`（`src/**`/`tests/**`/Schema/依赖零改动）。R4 head=本次提交。**待 Codex 复审 F（重点：R3-001 闭合是否成立、F=A+B 构成规则、60 组合证据）；ND-4/5/6/7 仍待用户裁决（ND-6 为 F 实现前待决）。**
- 历史：2026-09-19 R2 交付完成（R1 交付 A/B/C 三候选，实现 head=`5578a01`、契约 head=`ddaf865`；用户在 R1 交付后追加指示「再增加两种完全不一样的设计」→ R2 增补 D · Vermilion 朱砂与 E · Amber 琥珀，R2 head=`dd455c0`（参考+证据）、`af91e06`（契约），候选总数 3→5（AC② 口径经用户指示扩展，DDR-8）；R2 证据＝审计矩阵 50 组合 ALL PASS + 41 张截图）。同日 Review `db366da` 判 `changes_requested`（R-001 徽标对比度审计方式错误、R-002 命令面板契约缺口、R-003 D/E 切片缺口、R-004 overlap 未实现、R-005 审计日志证据纪律、R-006 R1 残留）→ R3 返修（六条全 closed：徽标真实合成底审计+亮色状态令牌加深、命令面板契约 §6B、D/E 切片扩展、真实 overlap 检测、Git Bash+venv 口径重跑、计数残留清理；head=`07397ac`）→ Review `07397ac` `approved`（唯一 P2 R3-001：直接 `--st-skip` 文本未入审计，open 移交后续）→ R4。base=`4c81dca`、branch=`agent/zcode/TASK-059-ui-redesign`、worktree=`G:/CODEX/New Manga.worktrees/TASK-059-zcode`。
