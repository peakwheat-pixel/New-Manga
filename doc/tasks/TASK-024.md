...
本 Task 的 4 条 AC 衡量设计交付；`doc/contracts/extensions.md` 原列 15 条 `AC-EXT-*` 产品验收草案，其中网页导入的 IMPORT-001/002 已按 U-1 退役。剩余 13 条仍是草案；U-2 已选定 PDFium via `pypdfium2`，但草案尚未转为正式 AC。两组 AC 属于不同层级，不得合并计数；产品草案不计入本 Task 的 4 条设计 AC。
...
- 最近状态：2026-09-17 窗口授权收口 `done`：设计 `d840075`、Review 修订 `86072c3`（`doc/reviews/TASK-024-design.md`，approved_subagent）、integration=`61c33e2`，master 复验 `536 passed`；用户裁决 U-1 取消网页导入、U-2 采用 PDFium via `pypdfium2`，已同步契约与追踪；U-3~U-6 仍待裁决，TASK-023/025/019/022 继续冻结。
