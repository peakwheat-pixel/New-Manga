...
- [x] **AC ①（根因）**：诊断并记录根因——planner 判定 Clean 可用性时看的是**stage 状态**而不是**当前 Clean artifact/指针的实际存在**。（TASK-019 的 handler 会提交 `ARTIFACT_CLEAN`，但 `clean` **stage** 未必被写成有效态。）**根因须以证据写明**（哪一行、为何误判），不得只改判据不谈机理。
