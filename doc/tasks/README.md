...
| [TASK-034](TASK-034.md) | 测试与分层硬化（route policy 收敛 / 架构守卫 / flaky 诊断） | DeepSeek Harness | ready；已按用户批准释放（Owner=DeepSeek Harness、Reviewer=Codex（**非作者**）、base=`b34b27e`、branch=`agent/deepseek/TASK-034-test-layer-hardening`、worktree=`G:/CODEX/New Manga.worktrees/TASK-034-deepseek`）；承接 TASK-019 尾项切片 Review 的 T-2/T-4、STATUS 两个 flaky 条目与 **TASK-035 R-02**（`tests/providers` conftest 收集顺序）；**AC ① 需先裁决**——`inpaint.route_policy` 设置语义在仓库内**无声明**（Codex 释放前实测），裁决前不得改行为；`src/` 改动需先申请范围变更；纯硬化、不改产品行为；实施未开始；TASK-019（done） |
