...
27 个任务均为 proposed / pending_user_review，无任务已认领或实施。**建议 Owner 不是实际派单。** 单个 Task 文件是状态真值，本表只提供标题和依赖导航。详细阶段与 Gate 见 [Roadmap](../12_ROADMAP.md)。

| Task | 交付目标 | 建议 Owner | 硬依赖 |
|---|---|---|---|
| [TASK-001](TASK-001.md) | 修订目标文档引用与已确定的一致性问题 | Codex | 用户审核后方可释放 |
