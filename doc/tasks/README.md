...
TASK-001～TASK-010、TASK-012、TASK-014、TASK-028 设计、TASK-029 统一 SQLite 持久化实现、TASK-030 入口装配与 TASK-031 生产 ImageDecoder/Managed Copy 适配器已完成并集成；TASK-011 已获授权并登记 `READY`、尚未实施；TASK-013、TASK-015～TASK-027 保持 `proposed`。**建议 Owner 不是实际派单。** 单个 Task 文件是状态真值，本表只提供标题和依赖导航。详细阶段与 Gate 见 [STATUS](../STATUS.md) 与 [Roadmap](../12_ROADMAP.md)。
...
| [TASK-011](TASK-011.md) | 实现命令计划、任务调度与可恢复进度 | Codex | ready；base `1668cd5`；TASK-008, TASK-009, TASK-010 |
