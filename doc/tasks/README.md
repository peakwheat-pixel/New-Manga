...
TASK-001～TASK-006 已完成并集成；TASK-007 为 `ready`，由 ZCode 承接；TASK-008～TASK-027 保持 `proposed`，尚未释放。**建议 Owner 不是实际派单。** 单个 Task 文件是状态真值，本表只提供标题和依赖导航。详细阶段与 Gate 见 [STATUS](../STATUS.md) 与 [Roadmap](../12_ROADMAP.md)。
...
| [TASK-007](TASK-007.md) | 实现书架领域与本地图片导入 | ZCode | ready；基线 `6b123fe`；TASK-006 |
