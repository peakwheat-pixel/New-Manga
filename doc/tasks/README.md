...
| [TASK-036](TASK-036.md) | 设置输入校验与导出发布顺序硬化（承接 TASK-034 R-02 / R-05 / R-07） | DeepSeek Harness | ready；已按用户批准释放（Owner=DeepSeek Harness、Reviewer=Codex（**非作者**）、base=`edfdcf2`、branch=`agent/deepseek/TASK-036-settings-validation-and-export-order`、worktree=`G:/CODEX/New Manga.worktrees/TASK-036-deepseek`）；承接 TASK-034 登记的三项仓库级遗留——**R-02**（`inpaint` 段本身非 mapping 静默回落）、**R-07**（字符串 `allowed_routes` 逐字符展开、`requirements` 非 bool 被 `bool()` 强制；**本轮解除冻结**）、**R-05**（`src/ui/viewmodels/export/viewmodel.py` 先清 `running` 后发布终态）；判据沿用 TASK-034 R-2 已确立的 fail-closed 原则，**无需再走裁决请求**；**不改路由判定算法**；不处理 R-03/R-06；实施未开始；TASK-034（done） |
