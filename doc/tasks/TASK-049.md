...
- [ ] **AC ①（detect 已注册且可诊断）**：`build_production_handlers` 注册 `detect` 步骤；未配置检测能力时返回**typed** 失败（错误码可诊断），不得静默成功。
- [ ] **AC ②（detection→Region 生产路径）**：新增/接通一条应用层路径：检测结果（既有 `ports/detection` 的 `DetectionResult` 语义）→ 落成 Region（复用 `RegionEditingService`/仓库纪律，含首 revision 与 reading_order），**不另造第二套 Region 写入**。
- [ ] **AC ③（端到端）**：页级 `ocr` 不再因"无 Region"失败：给出修前/修后对照（修前＝`INVALID_INPUT requires a Region target`；修后＝在"无检测器/无绑定"处 typed 失败），并说明 Region 产生后区域类命令的前进程度。
- [ ] **AC ④（不接 QML）**：QML 不改（`src/ui/qml/**` 零改动）；VM 侧可暴露 `createRegion` 槽但**不接线**，并在 Task 内说明"QML 入口待 TASK-047 设计门之后另开实现切片"。
- [ ] **AC ⑤（判别力 + 不回归）**：新增用例对修前代码失败；`tests/providers/**`、`tests/core/**` 既有断言逐条不变；全仓 passed 不减少；≥5 次逐次记录（不设 `QT_QPA_PLATFORM`）。
- [ ] **AC ⑥** Handoff + `verification/TASK-049/**` + 独立子对话 Review + 集成 + STATUS 台账行。
...
|---|---|---|---|---|
| AC ③ 修前对照 | `python verification/TASK-049/region_probe_pre.py`（planned） | venv + 真 SQLite | NOT_RUN | 无 |
| AC ①② 单测 | `pytest tests/providers tests/editing -q`（planned） | 同上 | NOT_RUN | 无 |
...
- Handoff：尚无。Review：尚无。实际测试：尚无（`ready`，前置 TASK-048）。
- **最近状态（当前，唯一）**：2026-09-19 由 Codex 依 §11 复核结论开立为 `ready`（窗口 W2）。**实施尚未开始。**
